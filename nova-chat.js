...
Build correct, clean, complete code for the user's specified language and build target. Start each file with a filename comment. Include concise setup instructions after the code. Do not use placeholder functions unless the user asks for a sketch.`,
...
function buildLiveContext(memory = []) {
...
Recent section memory (use only when it helps maintain context):
${memory.map((item) => `${item.role}: ${String(item.text).slice(0, 1000)}`).join("\n")}`;
}
...
async function askGroq(message, instructions, mode, memory) {
  // The previous NOVA deployment used Groq with llama-3.3-70b-versatile.
  // Groq retired that model for free/developer keys in August 2026, so keep
  // GROQ_MODEL configurable and use Groq's supported replacement by default.
...
async function askOpenAI(mode, message, instructions, memory) {
  const body = {
...
exports.handler = async (event) => {
  ...
  const memory = Array.isArray(payload.context) ? payload.context.slice(-6).map((item) => ({ role: String(item.role || "user").slice(0, 12), text: String(item.text || "").slice(0, 1000) })) : [];
  ...
  try {
    ...
    if (process.env.GROQ_API_KEY) {
      result = await askGroq(message, instructions, mode, memory);
    } else if (process.env.GEMINI_API_KEY) {
      result = await askGemini(mode, message, instructions);
    } else {
      result = await askOpenAI(mode, message, instructions, memory);
    }
